...
function user(email, pn, carrier){
	this.email=email;
	this.pn=pn;
...
$(document).ready(function(){
	...
	$("#send_btn").click(function(){
		...
		var data = "<files>";
		data += "<data ad=\""+bspeed_ad+"\" count=\"1\">"+bspeed+"</data>";
	...
	$("#submit_user").click(function(){
		...
		user_a = new user($("#email_a").val(), $("#pn_a").val(), $("#carrier_a").val());
		user_b = new user($("#email_b").val(), $("#pn_b").val(), $("#carrier_b").val());
		user_c = new user($("#email_c").val(), $("#pn_c").val(), $("#carrier_c").val());
		user_d = new user($("#email_d").val(), $("#pn_d").val(), $("#carrier_d").val());
		...
		for (i in users){
			...
			if(email_data != "<str>" && users[i].email.length>1){
				email_data += ","
			}
